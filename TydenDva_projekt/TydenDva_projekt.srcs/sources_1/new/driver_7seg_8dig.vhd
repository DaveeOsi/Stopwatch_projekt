...
begin
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                v_cnt := 0; s_ce <= '0';
            elsif v_cnt >= 250000 then
                v_cnt := 0; s_ce <= '1';
            else
                v_cnt := v_cnt + 1; s_ce <= '0';
            end if;
        end if;
    end process;

    -- Multiplexing(Přepínání mezi číslicemi)
    process(clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
    ...
    -- Logika výběru 
    process(s_cnt, data0_i, data1_i, data2_i, data3_i)
    begin
        dp_o <= '1'; 
        case s_cnt is
            when "000" =>
                dig_o <= "11111110"; s_hex <= data0_i;
            when "001" =>
                dig_o <= "11111101"; s_hex <= data1_i;
            when "010" =>
                dig_o <= "11111011"; s_hex <= data2_i;
                dp_o  <= '0'; 
            when "011" =>
                dig_o <= "11110111"; s_hex <= data3_i;
            when others =>
                dig_o <= "11111111"; s_hex <= "0000";
        end case;
    end process;
